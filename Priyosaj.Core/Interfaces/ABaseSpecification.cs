...
public abstract class ABaseSpecification<T> : ISpecification<T>
{
    ...
    protected void ApplySorting(string sort)
    {
        if (!string.IsNullOrEmpty(sort))
        {
            ...
            var propertyName =
                sort.Substring(0, 1).ToUpperInvariant()
            ...
            var property =
                targetType.GetRuntimeProperty(propertyName)
            ...
            var propertyReturningExpression = Expression.Lambda(
                Expression.Convert(Expression.Property(lambdaParamX, property), typeof(object)),
                lambdaParamX
            );
    ...
    protected void ApplyGroupBy(string groupBy)
    {
        ...
        {
            var propertyName =
                groupBy.Substring(0, 1).ToUpperInvariant()
            ...
            var property =
                targetType.GetRuntimeProperty(propertyName)
                ?? throw new InvalidOperationException(
                    $"Because the property {propertyName} does not exist it cannot be grouped by."
                );
